...
public class ObjectPool<T extends ObjectPool.Poolable> {
     ...
     */
    public static synchronized <P extends Poolable> ObjectPool<P> create(int withCapacity, P object) {
        ObjectPool<P> result = new ObjectPool<>(withCapacity, object);
        result.poolId = ids;
        ids++;
        return result;
    }
    ...
    private ObjectPool(int withCapacity, T object) {
        ...
        this.objects = new Poolable[this.desiredCapacity];
    ...
    private void resizePool() {
        ...
        Poolable[] temp = new Poolable[this.desiredCapacity];
